 ...
 */
 class CommonController extends Controller{
	 ...
	 public function _initialize(){
	      ...
	      if(!session('isLogin')||empty($admin_info)){
		  ...
		  //获取当前已保存的权限
		  $allPower=I('session.'.C('ADMIN_ACESS_LIST'));
		
		 	//进行权限判断
		   /* if(!in_array($controllerName,array_keys($allPower)) || !in_array($actionName,$allPower[$controllerName])){
	...
	public function del($img_name = false) {
			//删除指定记录
			$model = M(CONTROLLER_NAME);
			if (!empty($model)) {
				$pk = $model->getPk();
